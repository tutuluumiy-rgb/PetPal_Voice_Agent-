...
# 结果里出现这些强失败信号 → 任务未真正完成（不标 succeeded）。
# 注意不要收「白名单/不在工作区/未找到」等描述性词：worker 可能在遇到路径约束后
# 改用其他方式完成（补救成功仍是成功），文本里仍会提到这些词，会造成误判 failed。
_FAIL_SIGNALS = _re.compile(
    r"(任务失败|执行失败|无法完成|未能完成|失败：|错误：|都不能|最终失败|没有成功)"
)


def _looks_failed(text: str) -> bool:
    """判断 Worker 最终结果是否暗示任务确未完成（强失败信号；工具错误由 had_error 捕获）。"""
    t = _re.sub(r"(没有|无|未|不)[^\s，。；,.!?]{0,4}(失败|异常|错误)", "", text or "")
