...
## 9. 会话（Session）定义（用户确认，2025-03）

一次会话指「用户显式进入对话 → 用户显式结束」的完整对话期：

- **开始**：用户显式进入对话（唤醒词命中 / 手动开启麦克风对话）。
- **维持**：对话期间无论是否说话都算同一会话；**空闲回待机 = 静默挂起**——
  前端（`VoicePipeline._suspendToWake`）保留 WS 与后端会话、VAD 只监唤醒词；
  再次唤醒若连接仍在 → **恢复同一会话**（不重连、不新建，上下文/后台任务延续）。
  后端会话的生命周期 = 连接生命周期（长连接持续即会话持续）。
- **结束（均由用户触发）**：
  1. 口语退出（拜拜/再见/退出…）→ 前端断开连接，后端 `cleanup_session` 归档；
  2. 口语「新建会话」（重新开始/清除记忆…）→ 结束当前、重连开新会话；
  3. 显式 UI 操作（新建会话按钮 / 退出应用）。
- **长挂起钳底（防止僵尸连接）**：后端超过 `SESSION_HANGUP_IDLE_S`（默认 7200s/2h）
  无任何前端消息 → 后端自动结束该会话（再次唤醒将开新会话）。

> 语义要点：**长时间不说话不再开启新会话**；`session_id` 每次连接重新生成，
> 挂起复用同一连接 → 同一 `session_id` → 历史记录里一次完整对话 = 一条会话。
> 前端 VAD 人声判定仅在 `conversationStarted`（对话态）才上报
> `speech_start/speech_end`；待机/挂起态由唤醒词（KWS）接管。
